...
- (IBAction)filterTapped:(id)sender {
	UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:@"Filtering options" 
															 delegate:self 
													cancelButtonTitle:@"Cancel" 
											   destructiveButtonTitle:nil 
													otherButtonTitles: @"None-Rare", @"Resident Breeders", @"Summer Visitors", @"Winter Visitors", @"Passage Migrants", @"Introduced", @"All", nil];
...
- (void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex {
	// None rare selected
	if (buttonIndex == 0 && !([self.title isEqualToString:@"Non-Rare Birds"])) {
		[self setUpControllers:@"NonRare"];
	...
	if (buttonIndex == 1 && !([self.title isEqualToString:@"Residents"])) {
	...
	if (buttonIndex == 2 && !([self.title isEqualToString:@"Sum Visitors"])) {
	...
	if (buttonIndex == 3 && !([self.title isEqualToString:@"Winter Visitors"])) {
	...
	if (buttonIndex == 4 && !([self.title isEqualToString:@"Passage Migrants"])) {
		[self setUpControllers:@"PassageMigrant"];
		[self.tableView reloadData];
		self.title = @"Passage Migrants";
	}
	
	// Introduced selected
	if (buttonIndex == 5 && !([self.title isEqualToString:@"Introduced"])) {
	...
	// All selected
	if (buttonIndex == 6 && !([self.title isEqualToString:@"All Birds"])) {
